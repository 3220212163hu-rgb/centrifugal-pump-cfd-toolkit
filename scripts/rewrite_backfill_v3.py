...
def is_physics_variable(text):
    """判断是否是物理量变量（希腊字母或特定符号）"""
    # 希腊字母
...
def rewrite_paragraph_with_format(para, new_text):
    ...
    while remaining:
        ...
        # 匹配下标（紧跟在物理量变量后面的数字或字母）
        subscript_match = re.search(r'(?<=[αβγδεζηθικλμνξοπρστυφχψωΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩA-Za-z])([0-9]+)', remaining)
        ...
        if subscript_match:
            matches.append(('subscript', subscript_match.start(), subscript_match.end(), subscript_match.group()))
    ...
    elif parts[0][0] == 'subscript':
        set_run_format(first_run, is_subscript=True)
    else:
    ...
    for i, (part_type, part_text) in enumerate(parts[1:], 1):
        if i < len(para.runs):
            ...
            if part_type == 'ref':
                ...
                set_run_format(run, is_subscript=True)
            else:
                set_run_format(run)
