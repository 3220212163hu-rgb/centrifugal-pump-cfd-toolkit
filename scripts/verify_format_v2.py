...
def verify_paragraph_format(para, para_index):
    ...
    for i, run in enumerate(para.runs):
        ...
        # 检查下标（紧跟在物理量变量后面的数字或字母）
        subscript_match = re.search(r'(?<=[αβγδεζηθικλμνξοπρστυφχψωΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩA-Za-z])([0-9]+)', text)
        if subscript_match:
            subscript_text = subscript_match.group(1)
            if not run.font.subscript:
                issues.append(f"  Run {i}: 下标 '{subscript_text}' 不是下角标")
